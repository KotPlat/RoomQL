...
val q = query {
    ...
    where { UserEntityTable.age gte minAge }   // dropped when minAge is null
...
`query { }` returns a `RoomQlQuery` — a `sql: String` plus a positional `args: List<Any?>`. To run it through Room's `@RawQuery`, add [`roomql-runtime-android`](../runtime-android) for the `.toQuery()` bridge, and [`roomql-ksp-processor`](../ksp-processor) to generate the `*Table` column references.
